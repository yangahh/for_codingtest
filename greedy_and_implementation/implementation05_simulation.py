...
while True:
    ...
    if map[nx][ny] == 0:
        ...
        turn_time = 0
        continue
    # 다음에 갈 좌표가 바다(=1)이거나 가봤던 곳(=2)일 때 >> 왼쪽 방향으로 회전만(회전은 위에서 이미 했음)
    else: 
        turn_time += 1

    # 4바퀴를 다 돌았는데 갈 곳이 없을 때 뒤로 이동
    if turn_time == 4:
        # 뒤로 갈 좌표 설정. 뒤로 이동은 원래 가려던 만큼을 현재 위치에서 빼주면 된다.
        nx = x - dx[d]
        ny = y - dy[d]

        # 이동할 좌표가 이동 가능한 곳인 경우
        if map[nx][ny] == 0: 
            x = nx
            y = ny
            cnt += 1
        # 이동할 좌표가 바다이거나 가봤던 곳(=2)인 경우
        else: 
            break
        turn_time = 0
